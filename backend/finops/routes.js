...
router.post('/recommendations/:id/apply', async (req, res) => {
    ...
    try {
        ...
        const recResult = await pool.query(
            'SELECT * FROM finops_recommendations WHERE id = $1', [id]
        );
        ...
        try {
            ...
            console.warn(`Docker update for ${rec.container_name} failed (may not support live update):`, dockerError.message);
            // Continue anyway — mark as applied since the recommendation was acknowledged
        }

        // Mark as applied
        await pool.query(
            `UPDATE finops_recommendations SET status = 'applied', updated_at = NOW() WHERE id = $1`,
            [id]
        );
...
router.post('/recommendations/:id/dismiss', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `UPDATE finops_recommendations SET status = 'dismissed', updated_at = NOW() WHERE id = $1 RETURNING *`,
            [id]
        );
